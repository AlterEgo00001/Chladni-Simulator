...
const PARTICLE_COUNT_DEFAULT = 50000; // Увеличено для демонстрации мощи GPU
...
const PARTICLE_SIZE_DEFAULT = 1.5; // Размер в пикселях для gl_PointSize
...
const MAX_GRID_SIZE_DEFAULT = 251; // Увеличено для демонстрации мощи GPU
const FDM_STABILITY_FACTOR_DEFAULT = 0.08; 
const FDM_DAMPING_FACTOR_DEFAULT = 0.000050;
const BASE_FDM_STEPS_PER_FRAME_GPU = 20; // Оптимизировано для GPU
...
const ENABLE_FDM_OPTIMIZATION_DEFAULT = true; // Включено по умолчанию для GPU
...
// =================================================================================================
// ЧАСТЬ 1.2: ОПРЕДЕЛЕНИЕ КЛАССА И ЯДРО GPGPU
// Здесь определяется класс симулятора, его конструктор и ключевые методы для настройки
// GPGPU-конвейера: создание текстур для хранения данных и материалов для вычислений.
// =================================================================================================
...
class ChladniSimulatorGPU {
    constructor(loadedAssets) {
        ...
        // --- Инициализация всех свойств класса на основе констант ---
        ...
        this.BASE_MAX_FDM_STEPS_PER_FRAME = BASE_FDM_STEPS_PER_FRAME_GPU;
        ...
        this.playlistFiles = []; this.currentPlaylistIndex = -1;
        this.audioElement = null; this.audioFileSourceNode = null; this.audioFileBuffer = null;
        this.isAudioFilePlaying = false; this.isAudioFilePaused = false;
        this.audioFileDuration = 0; this.audioPlaybackOffset = 0; this.audioFileCurrentTime = 0;
        this.subtitleContainer = null; this.currentSubtitles = [];
        this.pitchDetectorAnalyserNode = null; this.pitchDetectorSignalBuffer = null;
        this.MIN_SAMPLES_FOR_PITCH_DETECTION = 0; this.MAX_SAMPLES_FOR_PITCH_DETECTION = 0;
        this.lastPitchUpdateTime = 0; this.lastStablePitchFrequency = 0; this.smoothedPitchFrequency = 273;
        this.isMicrophoneEnabled = false; this.microphoneStream = null; this.microphoneSourceNode = null;
        this.isDesktopAudioEnabled = false; this.desktopStream = null; this.desktopAudioSourceNode = null;
        ...
        this.bandSources = null; this.allDetectedBeats = []; this.currentBPM = 120;
        this.bpmUpdateIntervalId = null; this.BPM_PEAK_DETECTION_WINDOW = BPM_PEAK_DETECTION_WINDOW_DEFAULT;
        ...
        // GPGPU-специфичные свойства
        this.gpgpuScene = null; this.gpgpuCamera = null; this.gpgpuMesh = null;
        this.fdmStateA = null; this.fdmStateB = null; this.modalExcitationTexture = null;
        this.particleStateA = null; this.particleStateB = null; // WebGLMultiRenderTarget
        this.fdmUpdateMaterial = null; this.particleUpdateMaterial = null; this.particleRenderMaterial = null;
        this.dt_simulation_step = 0; this.dx = 0;
    ...
    _mainInitialization() {
        ...
        this._setupGPGPU(); // Критически важный новый метод
        
        // Финальная настройка UI и запуск
        this._createPianoKeys();
        this._setupEventListeners();
        this._resetAllSettingsToDefaults(false); // Применяем дефолтные настройки
    ...
    _setupThreeJSScene() {
        ...
        this.dirLight1.shadow.mapSize.width = 1024; this.dirLight1.shadow.mapSize.height = 1024;
        this.dirLight1.shadow.camera.near = 0.5; this.dirLight1.shadow.camera.far = this.PLATE_RADIUS * 10;
        this.dirLight1.shadow.camera.left = -this.PLATE_RADIUS * 2; this.dirLight1.shadow.camera.right = this.PLATE_RADIUS * 2;
        this.dirLight1.shadow.camera.top = this.PLATE_RADIUS * 2; this.dirLight1.shadow.camera.bottom = -this.PLATE_RADIUS * 2;
    ...
    _setupGPGPU() {
        // 1. Создание утилит для GPGPU (сцена, камера, полноэкранный квадрат)
        this.gpgpuScene = new THREE.Scene();
        this.gpgpuCamera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);
        const gpgpuGeometry = new THREE.PlaneGeometry(2, 2);

        // 2. Создание WebGLRenderTarget для хранения состояний симуляции
        const renderTargetOptions = {
            format: THREE.RGBAFormat,
            type: THREE.FloatType, // Критически важно для хранения данных высокой точности
        ...
        // FDM Render Targets (ping-pong)
        this.fdmStateA = new THREE.WebGLRenderTarget(this.MAX_GRID_SIZE, this.MAX_GRID_SIZE, renderTargetOptions);
        this.fdmStateB = new THREE.WebGLRenderTarget(this.MAX_GRID_SIZE, this.MAX_GRID_SIZE, renderTargetOptions);

        // Particle Render Targets (ping-pong)
        const particleTexSize = Math.ceil(Math.sqrt(this.MAX_PARTICLE_COUNT_USER_SETTING));
        this.particleStateA = new THREE.WebGLMultiRenderTarget(particleTexSize, particleTexSize, 2, renderTargetOptions);
        this.particleStateB = new THREE.WebGLMultiRenderTarget(particleTexSize, particleTexSize, 2, renderTargetOptions);
        
        this._initializeParticleTextures();
        
        // 3. Создание ShaderMaterial для каждого вычислительного шага
        
        // Материал для обновления сетки FDM
        this.fdmUpdateMaterial = new THREE.ShaderMaterial({
            uniforms: {
                ...
                u_exc_mode: { value: 0 },
                u_mParameter: { value: this.mParameter },
                u_excAmp: { value: 0.0 },
                u_simulationTime: { value: this.simulationTime },
                u_frequency: { value: this.actualAppliedFrequency },
            },
            vertexShader: this.shaders.common_vertex,
            fragmentShader: this.shaders.fdm_update_frag,
        });

        // Материал для обновления состояния частиц
        this.particleUpdateMaterial = new THREE.ShaderMaterial({
            uniforms: {
                u_particleStateTex: { value: this.particleStateA.texture },
                u_plateStateTex: { value: this.fdmStateA.texture },
                u_deltaTime: { value: 0.0 },
                u_forceMult: { value: this.PARTICLE_EFFECTIVE_FORCE_MULTIPLIER_BASE },
                u_damping: { value: this.PARTICLE_DAMPING_BASE },
                u_restitution: { value: this.PARTICLE_BOUNDARY_RESTITUTION },
                u_maxSpeed: { value: this.MAX_PARTICLE_SPEED },
            ...
            glslVersion: THREE.GLSL3, // Требуется для MRT
        });
        
        this.gpgpuMesh = new THREE.Mesh(gpgpuGeometry, this.fdmUpdateMaterial);
        this.gpgpuScene.add(this.gpgpuMesh);

        // Материал для финальной отрисовки частиц
        this.particleRenderMaterial = new THREE.ShaderMaterial({
            uniforms: {
                u_particlePositions: { value: this.particleStateA.texture[0] },
                u_plateState: { value: this.fdmStateA.texture },
        ...
        // 4. Создание объекта для видимых частиц
        ...
        this.particlesMesh = new THREE.Points(particleGeometry, this.particleRenderMaterial);
    ...
    _initializeParticleTextures() {
        ...
        const initialPassMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    outPos = texture(u_posTex, vUv);
                    outVel = texture(u_velTex, vUv);
                }
            `,
...
_setupAdvancedControlsListeners() {
    ...
    allConfigs.forEach(c => {
        ...
        const handleChange = (valFromUI) => {
            this._applyAdvancedSettingChange(c.varName, valFromUI);
        };

        if (iE) {
            const evT = (iE.type === 'checkbox') ? 'change' : 'input';
            iE.addEventListener(evT, (event) => {
                let valueToPass = c.type === 'checkbox' ? event.target.checked : event.target.value;
                handleChange(valueToPass);
            });
            if (iE.type === 'number') {
                iE.addEventListener('change', (event) => { handleChange(event.target.value); event.target.blur(); });
            }
        }
            ...
            sE.addEventListener('input', (event) => {
                ...
                if (c.type === 'float') {
                    if (c.exp != null && (Math.abs(sliderVal) > 1e5 || (Math.abs(sliderVal) < 1e-3 && sliderVal !== 0))) {
                        ...
                        displayString = sliderVal.toFixed(c.fixed || 3);
                    }
                } else if (c.type === 'int') {
                    displayString = Math.round(sliderVal).toString();
                }
                if (vE) vE.textContent = displayString;
            });
...
_applyAdvancedSettingChange(paramName, valStrBool) {
    let newVal;
    const inputElement = this.uiElements[paramName.replace(/([A-Z])/g, (match, p1) => `${p1.toLowerCase()}`).replace(/adv([A-Z])/, (match, p1) => `adv${p1}`)];
    const min = (inputElement && inputElement.min !== '') ? parseFloat(inputElement.min) : -Infinity;
    const max = (inputElement && inputElement.max !== '') ? parseFloat(inputElement.max) : Infinity;

    if (typeof valStrBool === 'boolean') {
        newVal = valStrBool;
    } else {
    ...
    if (Math.abs(previousValue - newVal) < 1e-9 && typeof previousValue === typeof newVal) {
        return; // No change
    }
...
_storeDefaultSimulationSettings() {
    this.defaultAdvancedSettings = {
        ...
        BASE_MAX_FDM_STEPS_PER_FRAME: BASE_FDM_STEPS_PER_FRAME_GPU,
...
async _resetAllSettingsToDefaults(isAdvancedOnly = false) {
    ...
    if (!isAdvancedOnly) {
        ...
        this.smoothedPitchFrequency = 273; this.lastStablePitchFrequency = 0;
...
// - Анализ звука в реальном времени (определение высоты тона, BPM).
// - Управление различными источниками звука (файл, микрофон, захват системы).
// - Работа с метаданными и субтитрами.
// =================================================================================================
...
// =================================================================================================
// ЧАСТЬ 4: УТИЛИТЫ, ЦИКЛ АНИМАЦИИ И ТОЧКА ВХОДА
// Этот финальный раздел содержит:
// - Вспомогательные функции (математика, физика, форматирование).
// - Логику виртуального пианино.
// - Методы для настройки UX (модальные окна, подсказки, локализация).
// - Главный цикл анимации `_animateScene`, который является сердцем симулятора.
// - Функцию `main`, которая запускает все приложение.
// =================================================================================================
...
_parseInputNumber(value, defaultValue = 0, isInt = false, minVal = -Infinity, maxVal = Infinity) {
    let num = isInt ? parseInt(value, 10) : parseFloat(value);
...
_findNthBesselRootFromTable(mOrder, nthRootOneIndexed) {
    ...
    const n0idx = n - 1;
    if (!this.besselRootsTable || !this.besselRootsTable.hasOwnProperty(m) || n0idx >= this.besselRootsTable[m].length) {
        return null;
    }
...
_findNthBesselRootNumerically(mOrder, nthRootOneIndexed) {
    ...
    while (currentX < maxX) {
        ...
        if(isNaN(currVal)) { currentX = nextX + step; continue; }
        if (prevVal * currVal < 0) {
            ...
            if (rootsFound === nthRootOneIndexed) {
                ...
                for (let i = 0; i < maxIter; i++) {
                    ...
                    if (Math.abs(high - low) < precision) break;
                    const midVal = this._besselJ_lib(mOrder, mid);
                    if (isNaN(midVal)) return (low+high)/2;
                    if (this._besselJ_lib(mOrder, low) * midVal < 0) high = mid; else low = mid;
                }
                return (low + high) / 2;
            }
        }
...
_updatePhysicalConstants() {
    ...
    if (!isFinite(this.D_FLEXURAL_RIGIDITY) || this.D_FLEXURAL_RIGIDITY <= 0) { this.D_FLEXURAL_RIGIDITY = 100; }
    if (!isFinite(this.RHO_H_PLATE_SPECIFIC_DENSITY) || this.RHO_H_PLATE_SPECIFIC_DENSITY <= 0) { this.RHO_H_PLATE_SPECIFIC_DENSITY = 10; }
}

_getOptimalGridSizeForFrequency(frequency) {
    let targetGridSize;
    const baseFreqMin = 200, freqMaxGrid = 7500, effFreq = Math.max(1, frequency);
...
_createPianoKeys() {
    ...
    keys.forEach(k => {
        ...
        if (k.t === 'white') { div.style.left = `${whiteOff * whiteW}px`; whiteOff++; } else { div.style.left = `${(whiteOff - 1) * whiteW + (whiteW - blackW / 2) - 1}px`; }
...
_handleKeyboardPiano(event) {
    if (!this.uiElements.pianoContainer || document.activeElement.tagName === 'INPUT' || document.activeElement.tagName === 'SELECT' || document.activeElement.tagName === 'TEXTAREA') {
        return;
    }
...
_animateScene() {
    ...
    if (!this.areParticlesFrozen) {
        ...
        this.gpgpuMesh.material = this.particleUpdateMaterial;
        this.particleUpdateMaterial.uniforms.u_particleStateTex.value = this.particleStateA.texture;
        this.particleUpdateMaterial.uniforms.u_plateStateTex.value = this.fdmStateA.texture;
